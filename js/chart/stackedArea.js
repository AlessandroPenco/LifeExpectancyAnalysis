function myAreaChart(entity) {
  ...
  // Parse the Data
  d3.csv(
    "https://raw.githubusercontent.com/AlessandroPenco/LifeExpectancyAnalysis/main/data/healthy-life-expectancy-and-years-lived-with-disability.csv"
  ).then(function (data) {
    //////////
    // GENERAL //
    //////////
    ...
    const x = d3
      .scaleLinear()
      .domain(
        d3.extent(entityData, function (d) {
          return d.Year;
        })
      )
      .range([0, width]);
    const xAxis = svgStaked
      .append("g")
    ...
    svgStaked.append("g").attr("class", "yAxis").call(d3.axisLeft(y).ticks(5));
    ...
    const areaChart = svgStaked.append("g").attr("clip-path", "url(#clip)");
    ...
    d3.selectAll("g.yAxis g.tick")
      ...
      .attr("stroke-width", 1.5); // make it dashed;;
  });
}
