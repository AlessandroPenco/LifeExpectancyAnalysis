...
function mapChart2(yy) {
  d3.select("#mapchart2").selectAll('svg').remove()
  d3.select("#mapchart2").selectAll('g').remove()
  // Map and projection
  ...
  Promise.all([
    ...
    var mappedData = world.objects.countries.geometries.map(function(d) {
      ...
      return {
        ...
        medianAge: +countryData.medianAge,
        geometry: d.geometry
      };
      else return {
        country: "unknown",
    ...
    var colorScale = d3.scaleSequential(d3.interpolateSpectral)
      .domain(d3.extent(mappedData, function(d) { return d.medianAge}));
    ...
    .enter()
      ...
      .style("fill", function(d) {
        ...
        return countryData ? colorScale(countryData.medianAge) : "grey";
      })
      ...
      .on("mouseover", function (event, d) {
        ...
        svgmapchart2.selectAll(`.${d.properties.name.replace(/\s+/g, '.')}`)
